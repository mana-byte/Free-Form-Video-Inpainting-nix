...
from model.networks import Generator, SNTemporalPatchGANDiscriminator


class VideoInpaintingModel(BaseModel):
    def __init__(self, opts, nc_in=5, nc_out=3, d_s_args={}, d_t_args={}):
        super().__init__()
        self.d_t_args = {"nf": 32, "use_sigmoid": True, "norm": "SN"}  # default values
        for key, value in d_t_args.items():
            # overwrite default values if provided
            self.d_t_args[key] = value

        self.d_s_args = {"nf": 32, "use_sigmoid": True, "norm": "SN"}  # default values
    ...
    def forward(self, imgs, masks, guidances=None, model="G"):
        ...
        if model == "G":
            ...
            raise ValueError(
                f'forwarding model should be "G", "D_t", or "D_s", but got {model}'
            )
