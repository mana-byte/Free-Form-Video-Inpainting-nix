...
class BaseTrainer:
    ...
    def __init__(
        ...
        writer_dir = os.path.join(
            config["visualization"]["log_dir"], config["name"], start_time
        )
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if epoch % self.save_freq == 0 or best:
                self._save_checkpoint(
                    epoch, save_best=best, monitor_value=monitor_value
                )
    ...
    def _save_checkpoint(self, epoch, save_best=False, monitor_value=None):
        ...
        state = {
            ...
            "optimizer_d_s": (
                self.optimizer_d_s.state_dict()
                if self.optimizer_d_s is not None
                else None
            ),
            "optimizer_d_t": (
                self.optimizer_d_t.state_dict()
        ...
        monitor_str = (
            f"-{self.monitor}{monitor_value:.4f}" if monitor_value is not None else ""
        )
        filename = os.path.join(
            self.checkpoint_dir, f"checkpoint-epoch{epoch}{monitor_str}{best_str}.pth"
        )
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        # Try loading state dict, skipping layers with size mismatch
        # try:
        #     self.model.load_state_dict(checkpoint["state_dict"], strict=False)
        # except RuntimeError as e:
        #     self.logger.warning(f"RuntimeError in load_state_dict: {e}")
        #     model_state = self.model.state_dict()
        ...
        #     }
        #     self.logger.warning(
        #         f"Loading only matching layers: {list(filtered_state.keys())}"
        #     )
    ...
    def _load_pretrained(self, pretrained_path):
        ...
        if self.pretrained_load_strict:
            ...
            self.logger.info(
                f"Submodule(s) not in pretrained model but in current model: {lack_modules}"
            )
            ...
            self.logger.info(
                f"Submodule(s) not in current model but in pretraired model: {set(redundant_modules)}"
            )
                ...
                if k in current_state:
                    ...
                    for prefix in prefixs:
                        ...
                        if new_key in current_state:
                            self.logger.warning(
                                f"Load key to new model: {k} -> {new_key}"
                            )
